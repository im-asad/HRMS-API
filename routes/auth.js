...
module.exports = sequelize => {
	...
	const { Employee, Role } = require('../models/')(sequelize)
	router.post('/verify-token', middlewares.verifyToken, async (req, res) => {
		const employee = await Employee.findOne({
			where: { machineCode: req.user.machineCode },
			include: [{ all: true }],
		})
	...
	router.post('/api/login', async (req, res) => {
		...
		const user = await Employee.findOne({ where: { username }, include: [Role] })
		...
		if (!user) {
			...
			if (match) {
				const { username, password, employeeName, employeeType, machineCode, role } = user
				const tokenUser = {
					username,
					password,
					employeeName,
